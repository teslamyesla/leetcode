...
|137|[Clone Graph](https://github.com/teslamyesla/leetcode/blob/master/python/lintcode-0137-clone-graph.py) |Medium| O(N+E)| O(N)|BFS in Graph|*| Classic Problem! Recurrsive or Iterative! Push each node to queue with BFS order, create new node and use visited to track mapping when pushing a node, update neighbors when poping the node and pushing the neighbors | 2020-12-24 |
...
|573|[Build Post Office II](https://github.com/teslamyesla/leetcode/blob/master/python/lintcode-0573-build-post-office-ii.py) |Hard| O(H*N*M), H is # of houses| O(N*M)| BFS in Graph|*| Try every empty place will TLE! Instead, BFS from every house, when reaching every empty place, update how many houses it has been touched and sum of dist from touched houses | 2020-12-23 |
|598|[Zombie in Matrix](https://github.com/teslamyesla/leetcode/blob/master/python/lintcode-0598-zombie-in-matrix.py) |Medium| O(N*M)| O(N*M)|Multi-Source BFS in Graph|*| Similiar as topology sort, queue all sources and propogate. 1) no need to keep visited tracker, can tell visited by value 2) days is # of levels, days - 1 is # of propogations | 2020-12-23 |
