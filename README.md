...
|375|[Guess Number Higher or Lower II](https://github.com/teslamyesla/leetcode/blob/master/python/0375-guess-number-higher-or-lower-ii.py) |Medium|O(n^2)|O(n^2)|DP, DFS, memo|*| memo[(start, end)], for x in the range, if guessing x you need to pay x + max(helper(i,x-1),helper(x+1,j)), take min cost | 2020-10-28 |
